...
data_dir = 'F:\ZZ\6LED-LC3\20200803base';
...
channel_name = ["Violet", "Green"]; 
channel_exposure = [ 150 1000 ];%ms
channel_level= [5 40]; % percent
...
%             well = well +1;
        ...
        for channel = 1:channel_num
            ...
            if channel == 2 % Only use for RFP: to turn on YG filter
            ...
            time_map(i) = now;
            if channel == 1
                for j = 1:2 % 20 frames x 150ms ~ 20seconds
                    mmc.snapImage();
            ...
            if channel == 1 % When previous channel is GFP, move to red emssion filter
                mmc.setProperty('LudlWheel', 'State', '3'); 
            elseif channel == 2 % When previous channel is RFP, turn off Lumencor's YG filter
