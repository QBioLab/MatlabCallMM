function [plate]=stitching(metainfo)
addpath ../lib
...
plate.folder_out = metainfo.well_plate.assembly_dir;
...
start_scan_point = floor(min(view_position_matrix_in_camera, [], 2));
end_scan_point =  ceil(max(view_position_matrix_in_camera, [], 2)); % find maxima in column
...
for ch_idx=1:plate.nc
    ...
    for view_idx=1:view_num
        ...
        view_position_in_assembly = view_position_in_camera - start_scan_point +1;% add 1 becuase matlab count from 1
        
        view_fname= sprintf("%s/%s", plate.view_dir, ...
            plate.view_fname_list(ch_idx, view_idx));
        raw_img=loadtiff(view_fname);
        raw_img = raw_img'; % because matlab is column-major-order
